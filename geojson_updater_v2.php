...
class GeoJsonUpdater {
    ...
    private function leerCSV() {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            if (count($row) > max($refIndex, $annoIndex, $dirIndex)) {
                ...
                // Limpiar datos UTF-8
                $refCat = $this->limpiarUTF8($refCat);
                $anno = $this->limpiarUTF8($anno);
                $direccion = $this->limpiarUTF8($direccion);
    ...
    private function leerGeoJSON() {
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            $this->log("GeoJSON decodificado correctamente después de limpieza UTF-8");
            return $data;
        }

        $this->log("Error decodificando GeoJSON después de limpieza: " . json_last_error_msg());
        ...
        foreach ($encodings as $encoding) {
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                $this->log("GeoJSON decodificado correctamente con codificación: $encoding");
                return $data;
            }
        }
    ...
    public function actualizarGeoJSON() {
        ...
        foreach ($geojsonData['features'] as &$feature) {
            if (isset($feature['properties']['REFCAT'])) {
                ...
                // Limpiar la referencia por si acaso
                $refCat = $this->limpiarUTF8($refCat);
        ...
        // Guardar el GeoJSON actualizado con flags para manejar UTF-8
...
if (php_sapi_name() === 'cli') {
    ...
    if ($result) {
        ...
        echo "Error en el procesamiento. Verifique el archivo de log." . PHP_EOL;
        exit(1);
    }
} else {
